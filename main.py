...
# CSV 파일 로드
df_F = pd.read_csv('1903F_Final.csv', encoding='cp949')
df_T = pd.read_csv('1930T_Final.csv', encoding='cp949')

# 질문과 답변 컬럼 추출
questions_F = df_F['Speaker_1'].tolist()
answers_F = df_F['Speaker_2'].tolist()
qa_dict_F = dict(zip(questions_F, answers_F))

questions_T = df_T['Speaker_1'].tolist()
answers_T = df_T['Speaker_2'].tolist()
qa_dict_T = dict(zip(questions_T, answers_T))
...
async def get_prediction(request: Request):
    ...
    if data in qa_dict_T:
        answer = qa_dict_T[data]
    ...
    for prediction in total_predictions:
        ...
        if (len(prediction) > 0) and (prediction != ' ') and (prediction != '  '):
...
async def get_prediction(request: Request):
    ...
    if data in qa_dict_F:
        answer = qa_dict_F[data]
        if len(answer) > len(predictions[0]):
            print(len(answer))
            print(len(predictions[0]))
            predictions[0] = answer
